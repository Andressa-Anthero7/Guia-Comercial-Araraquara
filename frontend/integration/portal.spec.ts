...
test("anunciante mantém sessão real e consegue entrar novamente após perder os cookies", async ({ page }) => {
  ...
  await test.step("anunciante envia capa e galeria, recarrega e remove a foto", async () => {
    await page.getByRole("button", { name: "Editar dados", exact: true }).click();
    await page.getByLabel("Trocar capa", { exact: true }).setInputFiles(path.resolve("public/anuncios/espetinhos.jpeg"));
    await expect(page.getByText("Preparando fotos...", { exact: true })).toHaveCount(0);
    await page.getByLabel("Adicionar fotos", { exact: true }).setInputFiles(path.resolve("public/anuncios/top-carros.jpeg"));
    await expect(page.getByAltText("Foto 1 da galeria", { exact: true })).toBeVisible();
    await expect(page.getByLabel("Adicionar fotos", { exact: true })).toBeDisabled();
    await page.getByRole("button", { name: "Salvar", exact: true }).click();
    await expect(page.getByRole("status")).toHaveText("Dados do estabelecimento atualizados.");
    const photos = await (await page.request.get("/api/advertiser/portal/")).json();
    expect(photos.businesses[0].image_url).toMatch(/^data:image\/webp;base64,/);
    expect(photos.businesses[0].gallery_images).toHaveLength(1);
    const catalog = await (await page.request.get("/api/businesses/contato-legado-de-teste/")).json();
    expect((await page.request.get(catalog.image_url)).status()).toBe(200);
    expect((await page.request.get(catalog.images[0].image)).status()).toBe(200);
    await page.reload({ waitUntil: "domcontentloaded" });
    await page.getByRole("button", { name: "Editar dados", exact: true }).click();
    await expect.poll(() => page.getByAltText("Foto 1 da galeria", { exact: true }).evaluate((img: HTMLImageElement) => img.naturalWidth)).toBeGreaterThan(0);
    await page.getByRole("button", { name: "Remover foto 1", exact: true }).click();
    await page.getByRole("button", { name: "Salvar", exact: true }).click();
    await expect(page.getByRole("status")).toHaveText("Dados do estabelecimento atualizados.");
    const removed = await (await page.request.get("/api/advertiser/portal/")).json();
    expect(removed.businesses[0].gallery_images).toHaveLength(0);
  });
