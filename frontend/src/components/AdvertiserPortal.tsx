...
import { AdvertiserMetrics } from "./AdvertiserMetrics";
...
export function AdvertiserPortal({ onLogout, onSessionExpired }: { onLogout: () => Promise<void>; onSessionExpired: () => void }) {
  ...
  const canLeave = () => !processingPhotos && !busy.current && (!dirty || window.confirm("Você tem alterações não salvas. Descartar as alterações e continuar?"));
  ...
  const save = async (action: () => Promise<unknown>, message: string) => {
    if (busy.current || processingPhotos) return false;
  ...
  return <AdvertiserWorkspace data={data} tab={tab} onTab={navigate} onLogout={() => void logout()} onRefresh={() => { if (canLeave()) { clearEditor(); void reload(); } }} loading={loading} saving={saving} busyMessage={leaving ? "Encerrando sessão…" : "Salvando alterações…"} error={error} notice={notice}>
    <div key={`${tab}-${reset}`}>
      ...
      {tab === "overview" && <AdvertiserMetrics metrics={data.metrics} />}
      {tab === "profile" && <ProfileSection data={data} {...editorProps} />}
      {tab === "businesses" && <BusinessesSection businesses={data.businesses} {...editorProps} renderPhotos={(business, onChange, disabled) => <BusinessPhotos cover={business.image} images={business.images ?? []} limit={business.benefits?.max_images ?? (business.planType === "paid" ? 5 : 1)} disabled={disabled} onProcessing={setProcessingPhotos} onChange={(image, images) => onChange({ ...business, image, images })} />} />}
      {tab === "ads" && <AdsSection advertisements={data.advertisements} {...editorProps} />}
      {tab === "coupons" && <CouponsSection coupons={data.coupons} businesses={data.businesses} {...editorProps} renderPhotos={(business, onChange, disabled) => <BusinessPhotos cover={business.image} images={business.images ?? []} limit={business.benefits?.max_images ?? (business.planType === "paid" ? 5 : 1)} disabled={disabled} onProcessing={setProcessingPhotos} onChange={(image, images) => onChange({ ...business, image, images })} />} />}
