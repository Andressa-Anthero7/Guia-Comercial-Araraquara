...
export function BusinessPhotos({ cover, images, limit, disabled, onChange, onProcessing }: Props) {
  ...
  return <fieldset disabled={locked} className="min-w-0 space-y-4 rounded-md border border-slate-200 p-4 sm:col-span-2">
    ...
    <div className="flex flex-wrap items-center gap-4">
      ...
      <label className="block min-w-0 max-w-full text-sm font-semibold">Trocar capa<input aria-label="Trocar capa" type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={event => void upload(event, true)} className="mt-2 block w-full min-w-0 text-xs" /></label>
    </div>
    ...
    <label className="block min-w-0 text-sm font-semibold">Adicionar fotos<input aria-label="Adicionar fotos" type="file" multiple accept="image/png,image/jpeg,image/webp,image/gif" disabled={locked || images.length >= limit} onChange={event => void upload(event, false)} className="mt-2 block w-full min-w-0 text-xs" /></label>
