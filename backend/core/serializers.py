...
class AdvertisementSerializer(ManagementValidationMixin, serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if validated_data.get("status") == Advertisement.Status.PUBLISHED:
            validated_data.update(has_published_version=True,
                published_starts_at=validated_data.get("starts_at"), published_ends_at=validated_data.get("ends_at"))
    ...
    def update(self, instance, validated_data):
        ...
        if next_status == Advertisement.Status.PUBLISHED:
            validated_data.update(has_published_version=True,
                published_starts_at=validated_data.get("starts_at", instance.starts_at),
                published_ends_at=validated_data.get("ends_at", instance.ends_at))
        elif next_status == Advertisement.Status.REVIEW and instance.status == Advertisement.Status.PUBLISHED:
            validated_data.update(has_published_version=True,
                published_starts_at=instance.starts_at, published_ends_at=instance.ends_at)
        elif next_status == Advertisement.Status.REVIEW and instance.status in (
            Advertisement.Status.PAUSED, Advertisement.Status.ENDED, Advertisement.Status.DRAFT):
            validated_data["has_published_version"] = False
