...
class PublicationAndFinanceTests(TestCase):
    ...
    def test_review_keeps_approved_profile_and_dates_until_approval(self):
        ad = Advertisement.objects.create(business=self.business, title='Aprovado', status='published',
            starts_at=self.today, ends_at=self.today + timedelta(days=1))
        self.business.description = 'Texto aprovado'
        self.business.save()
        self.client.force_authenticate(self.staff)
        response = self.client.patch(f'/api/backoffice/advertisements/{ad.pk}/', {
            'status':'review', 'description':'Ainda em revisão',
            'starts_at':str(self.today + timedelta(days=10)), 'ends_at':str(self.today + timedelta(days=20)),
        }, format='json')
        self.assertEqual(response.status_code, 200, response.data)
        self.assert_public(True)
        self.business.refresh_from_db()
        self.assertEqual(self.business.description, 'Texto aprovado')
        self.today += timedelta(days=2)
        self.assert_public(False)

    def test_revision_of_paused_campaign_does_not_republish_it(self):
        ad = Advertisement.objects.create(business=self.business, title='Pausado', status='paused', has_published_version=True)
        self.client.force_authenticate(self.staff)
        response = self.client.patch(f'/api/backoffice/advertisements/{ad.pk}/', {'status':'review'}, format='json')
        self.assertEqual(response.status_code, 200, response.data)
        self.assert_public(False)
    ...
    def test_all_plan_benefits_follow_active_contract_and_preserve_content(self):
        subscription = self.make_subscription()
        plan = subscription.plan
        for field in ('featured', 'includes_coupons', 'includes_marketing', 'includes_custom_page'):
            setattr(plan, field, True)
        plan.save()
        self.business.public_subdomain = 'empresa'
        self.business.meta_pixel_id = '123456789'
        self.business.is_featured = True
        self.business.save()
        with patch('core.benefits.timezone.localdate', return_value=self.today):
            data = self.client.get(f'/api/businesses/{self.business.slug}/').data
            self.assertEqual(data['public_subdomain'], 'empresa')
            self.assertEqual(data['meta_pixel_id'], '123456789')
            self.assertTrue(data['is_featured'])
            self.assertEqual(len(self.client.get(f'/api/coupons/?business={self.business.slug}').data), 1)
            self.client.force_authenticate(self.staff)
            ad = {'business': self.business.pk, 'title': 'Campanha', 'is_primary': False, 'status': 'draft'}
            for _ in range(plan.max_ads):
                self.assertEqual(self.client.post('/api/backoffice/advertisements/', ad, format='json').status_code, 201)
            self.assertEqual(self.client.post('/api/backoffice/advertisements/', ad, format='json').status_code, 400)
            oversized = {'images': ['https://example.com/1.png'] * (plan.max_images + 1)}
            self.assertEqual(self.client.patch(f'/api/backoffice/businesses/{self.business.slug}/', oversized, format='json').status_code, 400)
            for state in ('suspended', 'cancelled'):
                subscription.status = state
                subscription.save()
                self.client.force_authenticate(None)
                data = self.client.get(f'/api/businesses/{self.business.slug}/').data
                self.assertEqual(data['plan_type'], 'free')
                self.assertFalse(data['is_featured'])
                self.assertEqual(data['public_subdomain'], '')
                self.assertEqual(data['meta_pixel_id'], '')
                self.assertEqual(len(self.client.get(f'/api/coupons/?business={self.business.slug}').data), 0)
        self.business.refresh_from_db()
        self.assertEqual(self.business.public_subdomain, 'empresa')
        self.assertEqual(self.business.meta_pixel_id, '123456789')
