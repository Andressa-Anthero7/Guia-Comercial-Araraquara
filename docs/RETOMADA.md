...
**Atualização final de 22/09/2026:** versão 2.3.1 r2 publicada no site e na API; CI aprovado (65 API, 54 navegador, 3 integrações, 8 operacionais), dados preservados, DEBUG corrigido. Consulte [execução](EXECUCAO-2026-09-22.md) e [quadro](QUADRO-ENTREGA-GCA.xlsm). HTTPS dos subdomínios, PostgreSQL, SMTP e homologações externas ainda impedem aceite integral. Os registros abaixo são anteriores a essa execução.

**Retomada em 22/09/2026:** realizada [auditoria de funcionalidades e entrega](AUDITORIA-ENTREGA-2026-09-22.md). Produção ainda não aprovada: HTTPS incompatível nos dois subdomínios e páginas DEBUG públicas reapareceram. Backup periódico de hoje íntegro. Corrigidos localmente carregamento das telas/autenticação, TypeScript da seção de cupons, observação de artefatos Android, largura do formulário de fotos e confirmação de cópia de telefone. Novos testes de recuperação de senha e área de transferência. Sem deploy ou alteração de dados remotos. Preservar backlog anterior e alterações desta auditoria. Consultar relatório e evidências de 22/09 antes de usar os resultados históricos abaixo.
