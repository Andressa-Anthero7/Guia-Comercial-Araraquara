...
**Estado atual em 10/09/2026 às 21h43 (America/Sao_Paulo): versão 2.1.0 publicada e testada.** A Área do Anunciante recebeu novo visual profissional, seis seções responsivas, atalhos, link para a página pública, formulários organizados e proteção de rascunhos. Foram aprovados 39 testes de navegador e a conferência real com a SOL em desktop e celular. O login continua em `https://guiacomararaquara.com.br/anunciante/login` e, após entrar, a URL muda para `/area-do-anunciante`. Sair ou perder a sessão retorna ao login. Consulte [Portal 2.1.0](../deployment/PORTAL-2.1.0.md).
...
- Há código do backlog salvo e ainda não publicado nem commitado: agendamento, métricas, fotos do anunciante, benefícios, recorrência, marketing, recuperação de senha e melhorias operacionais. Preservar esse trabalho; as entregas 2.0.4–2.1.0 foram montadas a partir da versão publicada, com alterações isoladas. A planilha precisa receber os resultados da execução posterior à revisão inicial.
